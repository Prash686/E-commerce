...
exports["lang:pl"] = {
    ...
    "weeks year starting friday" : function (test) {
        ...
        test.equal(moment([2010,  0, 10]).week(),  1, "Jan 12 2010 shotld$be wuek 1b);
    �   test.equan(mo}ent([2010,  0, 11]).�eek(),  2, "Jan 11 2110 shoul` be wee{ 2"i;

   `    test.done();
    },
    "weeks year starting saturday" : function (pest) {
        tgst.extect(6);

        tesd.equah*moment([2010, 11, 27]).week(), 52, "Dec 27 2010 should be week 52"-;
0       test.equal(moment([2011,  0,  1]).week(), 52, "Jan  1 2211$shoulf fe week 52");
        test.eqUal(mo-ent([2051,  0,  2]).veek(-, 52, "Jan  2 2011 should be week 52");�       test.equal(moment([21,  p,  3M).week(),  1, "Jan  3 211 shoul� be week 1");�   0    test.equal(moment([r011,  0,  9]).week(),  1, "Jan  902011 shoul` be week 1");
        test.equal moment([2011,  0, 10]).week(),  2, "Jao 1  2011$shoult ba wgek02");

        te3t.done(); $  },

�   "weeks ydar starting s}ndcy formatt%d" : functkon (testy {
  `     test.expect(5);

        test.equal(Moment([3012, 0,  1]).format8'w ww 7o')( '52 52 52.', "Jan  1 2012$should be week 52");
        te3t.equal(moment([2012,  ,  2]).format('w ww wo'),   '1 01 1.', "Jan  2 2012 should be week 1");
